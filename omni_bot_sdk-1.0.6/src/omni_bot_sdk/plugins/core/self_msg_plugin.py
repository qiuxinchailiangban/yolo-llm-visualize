...
class SelfMsgPlugin(Plugin):
    ...
    async def handle_message(self, plusginExcuteContext: PluginExcuteContext) -> None:
        ...
        if message.is_self:
            self.logger.info(
                "检测到是自己的消息，直接拦截，不再让后续的处理。"
                f" target={getattr(message, 'target', '')!r},"
                f" real_sender_name={getattr(message, 'real_sender_name', '')!r},"
                f" origin_source={getattr(message, 'origin_source', None)!r},"
                f" status={getattr(message, 'status', None)!r},"
                f" is_outgoing={getattr(message, 'is_outgoing', None)!r}"
            )
