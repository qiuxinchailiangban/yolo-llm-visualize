...
class MessageSender:
    ...
    @staticmethod
    def _get_exact_center(bbox: List[float]) -> Tuple[int, int]:
        """返回 bbox 的精确中心点，避免随机漂移导致点偏。"""
        return (
            int((bbox[0] + bbox[2]) / 2),
            int((bbox[1] + bbox[3]) / 2),
        )
    ...
    def send_message(
        ...
        send_mode: Literal["enter", "button"] = "enter",
    ) -> bool:
        """
        发送文本消息。
        Args:
            ...
            send_mode (Literal["enter", "button"]): 发送方式，默认按回车发送。
        Returns:
            bool: 是否发送成功。
        """
        try:
            step_delay = max(getattr(self.window_manager, "action_delay", 0.3), 0.6)
            paste_delay = max(step_delay, 0.8)
            ...
            if send_mode == "enter":
                self.logger.info("使用回车发送消息")
                pyautogui.press("enter")
                time.sleep(step_delay)
                return True
            send_button = self.window_manager.get_icon_position("send_button")
            if send_button:
                center = self._get_exact_center(send_button)
